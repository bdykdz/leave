...
function bulletList(items) {
  return { type: 'bulletList', content: items.map(item => ({ type: 'listItem', content: [p(item)] })) }
}
function orderedList(items) {
  return { type: 'orderedList', attrs: { start: 1 }, content: items.map(item => ({ type: 'listItem', content: [p(item)] })) }
}
function hr() { return { type: 'horizontalRule' } }
function blockquote(text) { return { type: 'blockquote', content: [p(text)] } }
function doc(...content) { return { type: 'doc', content } }

const BLOCK_NODES = new Set(['paragraph','heading','bulletList','orderedList','listItem','blockquote','codeBlock','horizontalRule'])
function extractPlainText(node) {
  ...
  if (node.content && Array.isArray(node.content)) {
    for (const child of node.content) {
      ...
      if (BLOCK_NODES.has(child.type)) text += '\n'
    }
  }
  return text
}
...
async function createPage(data) {
  ...
  const page = await prisma.wikiPage.create({
    data: {
      ...
      translations: {
        create: { language: 'ro', title: data.title, content: data.content, contentText, excerpt },
      },
      revisions: {
        create: { language: 'ro', version: 1, title: data.title, content: data.content, authorId: data.authorId, changeNote: 'Versiunea inițială' },
      },
...
async function main() {
  ...
  const catAngajat = await prisma.wikiCategory.create({
    data: { name: 'Ghid Angajat', nameEn: 'Employee Guide', nameRo: 'Ghid Angajat', slug: 'ghid-angajat', icon: '👤', sortOrder: 2, description: 'Ghid pentru angajați' },
  })
  const catManager = await prisma.wikiCategory.create({
    data: { name: 'Ghid Manager', nameEn: 'Manager Guide', nameRo: 'Ghid Manager', slug: 'ghid-manager', icon: '👥', sortOrder: 3, description: 'Ghid pentru manageri' },
  })
  const catHR = await prisma.wikiCategory.create({
    data: { name: 'Ghid HR', nameEn: 'HR Guide', nameRo: 'Ghid HR', slug: 'ghid-hr', icon: '🏢', sortOrder: 4, description: 'Ghid pentru departamentul HR' },
  })
  const catReguli = await prisma.wikiCategory.create({
    data: { name: 'Reguli și Politici', nameEn: 'Rules & Policies', nameRo: 'Reguli și Politici', slug: 'reguli-politici', icon: '📋', sortOrder: 5, description: 'Regulamentul intern privind concediile' },
  })
  ...
  console.log('\n✅ Wiki seeded with 11 pages, 5 categories, 4 tags')
}
