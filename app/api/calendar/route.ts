...
export const GET = asyncHandler(async (request: NextRequest) => {
  ...
  const calendarEvents = [
    ...leaveRequests.map(leave => ({
      ...
      type: 'leave' as const,
      userId: leave.userId,
      userName: `${leave.user.firstName || ''} ${leave.user.lastName || ''}`.trim(),
      userAvatar: leave.user.profileImage,
      userInitials: `${(leave.user.firstName || 'U')[0]}${(leave.user.lastName || '')[0]}`,
    ...
    ...wfhRequests.map(wfh => ({
      ...
      type: 'wfh' as const,
      userId: wfh.userId,
      userName: `${wfh.user.firstName || ''} ${wfh.user.lastName || ''}`.trim(),
      userAvatar: wfh.user.profileImage,
      userInitials: `${(wfh.user.firstName || 'U')[0]}${(wfh.user.lastName || '')[0]}`,
