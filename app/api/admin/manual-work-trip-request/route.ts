...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isNaN(parsedTotalDays) || parsedTotalDays <= 0 || parsedTotalDays > 366) {
      return NextResponse.json({ error: 'totalDays must be a positive number (max 366)' }, { status: 400 })
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (targetUser.managerId) {
        ...
        await tx.notification.create({
          data: {
            ...
            message: `${targetUser.firstName} ${targetUser.lastName} has requested a ${parsedTotalDays} day(s) work trip to ${sanitizedDestination} (created by Admin)`,
      ...
      await tx.notification.create({
        data: {
          ...
          message: `A work trip request for ${parsedTotalDays} day(s) from ${startDateISO} to ${endDateISO} to ${sanitizedDestination} has been created by Admin and sent to your manager for approval.`,
          link: '/employee',
        },
      })
    ...
    await createAuditLog({
      ...
      metadata: {
        reason: `Admin manual work trip entry for ${targetUser.firstName} ${targetUser.lastName}`,
