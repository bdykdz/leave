...
export async function GET(request: Request) {
  try {
    ...
    // Get requests that this manager has approved (regardless of overall status)
    ...
    const formattedRequests = approvedRequests.map(request => {
      ...
      // Check if request is fully approved or still pending other approvals
      ...
      return {
        ...
        overallRequestStatus: request.status // Add the actual request status
      }
    })
